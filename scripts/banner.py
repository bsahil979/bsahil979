#!/usr/bin/env python3
"""Generate the animated GitHub profile banners for Sahil Belchada,
faithfully matching the https://github.com/emmi-lili visual map architecture.
"""
...
ROOT = Path(r"C:\Users\Sahil Belchada\.gemini\antigravity-cli\brain\efca0422-ce71-4d42-abbd-9fd898372e2c\scratch\bsahil979")
SOURCE = Path(r"C:\Users\Sahil Belchada\Desktop\portfolio\assets\sahil.jpg")
ASSETS = ROOT / "assets"
LOGOS = ROOT / "scripts/banner/logos"
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # 1. Distributed Systems Gear (represents high-throughput streaming & backend systems)
    ...
    # 2. </> Code mark built from broad, rounded strokes
    ...
    # 3. AI Neural Network / Star Mark (representing AI agents & ML pipelines)
    ai_star = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(ai_star)
...
def segment_subject(rgb_im: Image.Image) -> np.ndarray:
    """Flood-fill segmentation to cleanly isolate subject from off-white studio background."""
    w, h = rgb_im.size
    rgb_arr = np.asarray(rgb_im, dtype=np.float32)
    gray = np.mean(rgb_arr, axis=2)
    visited = np.zeros((h, w), dtype=bool)
    is_bg = np.zeros((h, w), dtype=bool)
    q = deque([(0, 0), (0, w - 1), (5, 0), (5, w - 1)])
    for y, x in q:
        visited[y, x] = True

    while q:
        cy, cx = q.popleft()
        if gray[cy, cx] > 200 and abs(rgb_arr[cy, cx, 0] - rgb_arr[cy, cx, 2]) < 25:
            is_bg[cy, cx] = True
...
def render_svg(
    ...
    systems = transport(source, logo_points["systems"][:n])
    code = transport(systems, logo_points["code"][:n])
    ai_star = transport(code, logo_points["ai"][:n])

    times = [0, 3.0, 4.3, 6.3, 7.6, 9.6, 10.9, 12.9, 14.2]
    key_times = ";".join(num(v / LOOP_SECONDS) for v in times)
    frames = [source, source, systems, systems, code, code, ai_star, ai_star, source]
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Sahil Belchada's live system profile</title>",
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        "systems, code, and AI silhouettes.</desc>",
        "<defs>",
    ...
    # Dense portrait drift: 94 independently noisy bands moving toward Systems centroid
    systems_centroid = systems.mean(axis=0)
    ...
    for band in range(94):
        ...
        delta = (systems_centroid - centroid) * 0.18 + noise[band]
        d = point_path(pts)
        parts.append(
    ...
    parts.extend(
        [
            ...
            'font-weight="700">@bsahil979</text>',
        ]
    )
